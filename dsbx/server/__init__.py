...
- ``__main__``: ``python -m dsbx.server`` entry point (used by
  ``dsbx serve``).
"""
...
def make_app(backend):  # pragma: no cover - thin re-export
    """Re-export so callers can ``from dsbx.server import make_app``."""
    from dsbx.server.app import make_app as _make
