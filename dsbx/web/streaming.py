...
``dsbx/server/app.py`` already emits, so the browser's SSE parser
is the same as the one inside ``dsbx/backends/remote.py``.
...
from dsbx.core.engine import generate as core_generate
from dsbx.core.samplers import Sampler
from dsbx.core.speculative import speculative_generate
from dsbx.server.schemas import genstep_to_wire, step_to_wire

log = logging.getLogger("dsbx.web.streaming")


def sse_frame(payload: dict) -> bytes:
    """Encode one SSE frame -- ``data: <json>\\n\\n``.

    Identical to the encoder in :mod:`dsbx.server.app`. Kept
...
def stream_generate(
    ...
    *,
    ...
    # :class:`dsbx.core.usage.UsageAware` (OpenAICompatBackend
    ...
    # :mod:`dsbx.web.backends` makes this concurrency-safe.
...
def _can_use_native_cloud_stream(
    ...
    :class:`~dsbx.backends.openai_compat.OpenAICompatBackend`
...
def stream_spec(
    ...
    *,
    ...
    """
    from dsbx.server.schemas import candidate_to_wire
