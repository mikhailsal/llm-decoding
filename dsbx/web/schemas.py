...
do NOT mirror :mod:`dsbx.core.config` -- a public ``BackendInfo``
...
the converters from :mod:`dsbx.server.schemas` rather than
...
- ``GenerateRequest`` re-uses :class:`dsbx.server.schemas.SamplerSpec`
...
from dsbx.server.schemas import (
    SamplerSpec,
    WireCapabilities,
    WireGenStep,
    WireStepResult,
...
class ManualTranscript(BaseModel):
    ...
    Mirrors :meth:`dsbx.core.manual.ManualSession.to_dict` but
...
class WireSpecRound(BaseModel):
    """Mirror of :class:`dsbx.core.speculative.SpecRound`."""
