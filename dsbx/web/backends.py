...
mirroring the locking policy that ``dsbx/server/app.py`` already
...
- Speculative-decoding HF target/draft pairs. Those are heavy and pin VRAM;
  ...
  stream ends (mirrors :func:`dsbx.cli.app.cmd_spec`).
...
if TYPE_CHECKING:
    from dsbx.server.schemas import WireCapabilities
...
log = logging.getLogger("dsbx.web.backends")
