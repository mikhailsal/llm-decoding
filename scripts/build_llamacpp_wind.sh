#!/usr/bin/env bash
# Build llama.cpp with CUDA on `dsbx-host` for the P40 (compute capability 6.1).
#
# Notes for this box:
...
# - We target only sm_61 (the P40) to keep the build small and fast.
# - LLAMA_CURL is off: we always pass a local GGUF path, no remote fetch needed.
#
# Usage on dsbx-host:  bash scripts/build_llamacpp_wind.sh
set -euo pipefail
